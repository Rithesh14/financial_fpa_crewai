...
- 🖥️ **Streamlit Web Dashboard**: Interactive UI with file uploading, configuration, and a detailed results viewer.
...
- 🛡️ **Deterministic Fallbacks**: Generates an automated, structured report even if the LLM API is unavailable or rate limits are hit.
...
* ⚠️ **Financial Risk Analyst**: Evaluates liquidity, leverage, and flags potential risks.
...
> **🔥 MAXIMIZING PERFORMANCE WITH GROQ**
>
> We strongly recommend using the **Groq API** with the `meta-llama/llama-4-scout-17b-16e-instruct` model. This specific model provides the best balance of speed and reasoning while offering the **maximum tokens per minute (TPM)** limit, ensuring your multi-stage financial reports run smoothly without hitting rate limit bottlenecks.
...
2. **Upload your financial CSV dataset.**
   ...
   > The uploaded CSV file **must strictly follow** the structure and formatting of the built-in demo dataset (`src/data/modified_financial_data.csv`).
   >
   ...
   >
   > Ensure your column names exactly match and data types are consistent. *(A built-in demo dataset is available to test the application immediately).*
...
- 🗄️ `cache/`: Hashes of previous analyses to optimize execution speed on re-runs.
